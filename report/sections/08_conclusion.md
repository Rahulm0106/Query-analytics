...
Three extensions: (i) re-train XGBoost with `nrounds` ≥ 1,500 to close the CV convergence gap; (ii) fit a boundary-aware Radius sub-model conditioned on `boundary_flag` to address the 99.9% boundary-crossing rate; (iii) benchmark a learned-index baseline (e.g. MSCN, Kipf et al. 2019) to test whether deep features add value beyond hand-engineered geometric ones. The pipeline is deterministic (seed = 42) and runs end-to-end in under an hour on commodity hardware.
